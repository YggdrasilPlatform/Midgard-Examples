...
static inline void render_scanline_bg(struct GB_Core* gb,
    ...
    uint8_t* pixels = (uint8_t*)gb->ppu.pixles[scanline];
    ...
    for (uint8_t palette = 0; palette < 8; ++palette) {
        if (dirty[palette] == true) {
            ...
            for (uint8_t colours = 0, pos = 0; colours < 4; colours++, pos += 2) {
                ...
                // this is i bgr remember!

                // NOTE: this might be wrong, cant remember if its &F or &1f, try both!
                uint8_t r = (pair >> 0x0) & 0x0F;
                uint8_t g = (pair >> 0x5) & 0x0F;
                uint8_t b = (pair >> 0xA) & 0x0F;

                r >>= 2;
                g >>= 2;
                b >>= 3;

                // put it back in rgb format so its more sane for my brain
                map[palette][colours] = (r << 5) | (g << 2) | (b << 0);
            }
        }
    }
}
